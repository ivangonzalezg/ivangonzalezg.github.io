...
export const Navbar: React.FC = () => {
  ...
  return (
    <>
      <nav
        className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 border-b py-5 ${
          isScrolled
            ? "bg-white/90 backdrop-blur-md border-slate-200"
            : "bg-transparent border-transparent"
        } ${
          isScrolled && isMobileMenuOpen && "bg-white/90 border-transparent"
        }`}
      >
      ...
      {/* Mobile Menu Overlay sits outside nav to avoid nav stacking context */}
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
